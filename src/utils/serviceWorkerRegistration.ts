...
export function registerSW(config?: Config) {
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', () => {
      const swUrl = `${process.env.PUBLIC_URL}/sw-performance.js`;
...
export function initializePerformanceOptimization() {
  ...
  // Listen for performance optimization messages
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.addEventListener('message', (event) => {
      if (event.data && event.data.type === 'CACHE_STRATEGY_UPDATED') {
        console.log('Cache strategy updated:', event.data.strategy);
        
        // Dispatch custom event for components to listen
        window.dispatchEvent(new CustomEvent('performance-optimized', {
          detail: { strategy: event.data.strategy }
        }));
      }
    });
  }
